...
Reference Manual {revnumber}. For
...
the tajima DST format cite:[4] for existing printers and CNC commands for
direct control... where'd we'd use G-Code cite:[7] and Linux CNC cite:[6].
...
We maintain a traditional manpage for `embroider` using
the basic macros.
...
_Embroidermodder 2_ is very much a work in progress since we're doing a ground
...
* This reference manual covering the development of all these projects with the current version available here: https://www.libembroidery.org/embroidermodder_2.0_manual.pdf[]
* The GUI _Embroidermodder 2_ covered in Chapter~\ref{GUI}.
...
maintained in cite:[embroidermodder_website].
